...
// Root aliases in capability_cache.go (package main) keep existing call
// sites unchanged.
...
// and nil is returned.
func (c *CapabilityCache) Get(agentID string) *capability.Payload {
	c.mu.RLock()
...
func (c *CapabilityCache) Set(agentID string, payload capability.Payload, ttl time.Duration) {
	if ttl <= 0 {
...
func (c *CapabilityCache) List() map[string]capability.Payload {
	c.mu.RLock()
	defer c.mu.RUnlock()

	now := time.Now()
	result := make(map[string]capability.Payload, len(c.entries))
