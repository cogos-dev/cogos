...
var (
	...
	// ErrTransport means the signature material could not be RETRIEVED — a
	// timeout, a DNS failure, a 5xx from the CDN, a VPN that blackholes
	// github.com. It is deliberately NOT ErrVerification.
	//
	// Conflating the two is how a network outage comes to look like an attack.
	// The operator-visible message and the correct response differ completely:
	// a verification failure is terminal and means someone handed us bad
	// material, while a transport failure means we learned nothing at all and
	// should try again next cycle. Under enforce both stop the update — but
	// only one of them should say so in the language of compromise.
	ErrTransport = errors.New("could not retrieve signature material")
)
...
var (
	...
	//
	// DEPRECATED BY FULCIO. The 1.1–1.6 block is marked deprecated in Fulcio's
	// OID spec in favour of the 1.8+ DER-encoded equivalents; 1.5's successor is
	// oidSourceRepoURI (1.12). It is still emitted today (confirmed present in
	// the v0.16.26 leaf) and is kept as a fallback, but it must never be the
	// sole basis for a hard failure — see verifySourceRepository.
	oidSourceRepo = asn1.ObjectIdentifier{1, 3, 6, 1, 4, 1, 57264, 1, 5}
	// oidSourceRepoURI (1.12) is the DER-wrapped successor to 1.5, holding the
	// full URI form ("https://github.com/owner/name").
...
func (p *Policy) verifyChain(cert *x509.Certificate) error {
	// A nil Roots pool is not "no constraint" — x509.Verify documents it as
	// "use the host's system trust store", which would silently accept any
	// certificate a public CA will issue. PolicyForRelease always populates the
	// pool, so reaching here with nil means a caller hand-built a Policy and
	// forgot; fail rather than fall back to a trust root nobody chose.
	if p.Roots == nil {
		return fmt.Errorf("%w: policy has no pinned trust root (refusing to fall back to the system store)", ErrVerification)
	}
...
func (p *Policy) verifyIdentity(cert *x509.Certificate) (*Attestation, error) {
	...
	sourceRepo, err := p.verifySourceRepository(cert)
	if err != nil {
		return nil, err
	}
...
// verifySourceRepository enforces the source-repository pin, if the
// certificate asserts one.
//
// TWO DISTINCTIONS THAT MATTER, both learned the hard way:
//
// 1. PREFER 1.12 OVER 1.5. Fulcio deprecated the 1.1–1.6 extension block in
// favour of the DER-encoded 1.8+ forms. Both are present in certificates issued
// today, but only 1.12 is the supported one going forward. Reading 1.5 first
// would make this verifier depend on an extension Sigstore has already said it
// intends to stop emitting.
//
// 2. ABSENT IS NOT MISMATCHED. If neither extension is present, the certificate
// simply does not assert a source repository, and this check has nothing to
// say. Treating that as a failure means the day Sigstore drops the deprecated
// block, every node on `enforce` stops updating simultaneously — with a message
// that reads like an attack in progress. That is a self-inflicted fleet-wide
// outage dressed up as a security control.
//
// Refusing on absence would also be redundant: the SAN pin in verifyIdentity
// already binds owner AND name AND workflow AND ref exactly. This check exists
// only as defence in depth should that pattern ever be loosened by mistake, so
// it costs nothing to make it advisory when unasserted and strict when
// asserted.
func (p *Policy) verifySourceRepository(cert *x509.Certificate) (string, error) {
	if p.SourceRepository == "" {
		got, _ := certExtension(cert, oidSourceRepoURI, true)
		return strings.TrimPrefix(got, "https://github.com/"), nil
	}

	// Preferred: 1.12, the DER-wrapped URI form.
	if uri, ok := certExtension(cert, oidSourceRepoURI, true); ok {
		want := "https://github.com/" + p.SourceRepository
		if uri != want {
			return "", fmt.Errorf("%w: certificate source repository URI %q does not match required %q",
				ErrVerification, uri, want)
		}
		return p.SourceRepository, nil
	}

	// Fallback: 1.5, the deprecated bare "owner/name" form.
	if legacy, ok := certExtension(cert, oidSourceRepo, false); ok {
		if legacy != p.SourceRepository {
			return "", fmt.Errorf("%w: certificate source repository %q does not match required %q",
				ErrVerification, legacy, p.SourceRepository)
		}
		return legacy, nil
	}

	// Unasserted. The SAN pin already carries this constraint; see above.
	return "", nil
}
