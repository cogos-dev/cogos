...
// TestToolDispatchToHarness_HarnessProviderDefault verifies that when
// cfg.HarnessProvider is set and the caller does not pass an explicit provider,
// the harness_provider value is applied as the default before the request
// reaches the dispatcher.
func TestToolDispatchToHarness_HarnessProviderDefault(t *testing.T) {
	t.Parallel()
	root := makeWorkspace(t)
	cfg := makeConfig(t, root)
	...
	srv := NewMCPServerWithAgentController(cfg, makeNucleus("Cog", "tester"), nil, disp)

	// No explicit provider in the input.
	_, _, err := srv.toolDispatchToHarness(context.Background(), nil, dispatchToHarnessInput{
		Task: "test task",
	})
	if err != nil {
		t.Fatalf("toolDispatchToHarness: %v", err)
	}
	if disp.lastReq.Provider != "lmstudio" {
		t.Errorf("Provider = %q; want \"lmstudio\" (from HarnessProvider default)", disp.lastReq.Provider)
	}
}

// TestToolDispatchToHarness_ExplicitProviderOverridesHarnessProvider verifies
// that an explicit provider on the request takes precedence over
// cfg.HarnessProvider.
func TestToolDispatchToHarness_ExplicitProviderOverridesHarnessProvider(t *testing.T) {
	t.Parallel()
	root := makeWorkspace(t)
	cfg := makeConfig(t, root)
	cfg.HarnessProvider = "lmstudio"
	...
	// Caller supplies an explicit provider — should win over HarnessProvider.
	_, _, err := srv.toolDispatchToHarness(context.Background(), nil, dispatchToHarnessInput{
		Task:     "test task",
	...
	if disp.lastReq.Provider != "claude-code" {
		t.Errorf("Provider = %q; want \"claude-code\" (explicit override wins)", disp.lastReq.Provider)
	}
}

// TestToolDispatchToHarness_EmptyHarnessProviderLeavesLegacyPath verifies that
// when cfg.HarnessProvider is empty, the request's Provider is not modified,
// preserving the legacy Ollama-probe fallback.
func TestToolDispatchToHarness_EmptyHarnessProviderLeavesLegacyPath(t *testing.T) {
	t.Parallel()
	root := makeWorkspace(t)
	cfg := makeConfig(t, root)
	...
	if disp.lastReq.Provider != "" {
		t.Errorf("Provider = %q; want empty (empty HarnessProvider = unchanged legacy path)", disp.lastReq.Provider)
	}
}
