...
type dispatchToHarnessInput struct {
	...
	Model        string                 `json:"model,omitempty" jsonschema:"Inference backend hint. Legacy values \"e4b\" and \"26b\" are accepted for backward compat and route through the local-LLM probe. Prefer setting provider=\"lmstudio-darkstar\" for explicit routing. Ignored when provider is set."`
	Provider     string                 `json:"provider,omitempty" jsonschema:"Named provider override (e.g. \"lmstudio-darkstar\", \"anthropic\"). Must match a provider declared in providers.yaml or providers.local.yaml; unknown names error before any slot runs. When set, takes precedence over model. Per RFC-0007."`
