...
type dispatchToHarnessInput struct {
	...
	Model        string                 `json:"model,omitempty" jsonschema:"Inference backend hint or explicit model id. Legacy values \"e4b\" and \"26b\" route through the local-LLM probe unchanged. Any other value (e.g. \"ornith-1.0-35b\") is treated as an explicit model request: it wins over the resolved provider's configured model default and is sent to the provider verbatim. If the provider cannot serve it, the dispatch fails loudly (provider error surfaced in the slot's error field) rather than silently substituting. Per issue #430."`
