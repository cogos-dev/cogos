...
//   - string concatenation (a + b)                       -> resolve each side, concatenate (an empty-resolved left side glued to a right side that itself starts with "/" is flagged degenerate — see degenerateRootMarker — the same over-claim filepath.Join(emptyBase, name) used to produce)
...
// fallback.
// degenerateRootMarker prefixes a pattern produced by concatenating a
// left operand that resolved to the literal empty string with a
// right-hand literal that itself begins with "/" (see resolveExpr's
// BinaryExpr case). It flags that the pattern's apparent leading slash is
// an artifact of manual path-building, not a positively-established
// absolute-path root — the concatenation counterpart of the leading-slash
// artifact the filepath.Join handler in resolveCall normalizes away by
// skipping empty arguments before joining. Chosen to be a string no real
// Go source or resolved path segment will ever contain, so it can be
// tested for with a plain strings.HasPrefix without colliding with a
// genuine pattern.
const degenerateRootMarker = "\x00degenerate-root\x00"
...
func (r *resolver) resolveExpr(e ast.Expr, defs map[string]ast.Expr, depth int) (pattern string, ok bool) {
	...
	case *ast.BinaryExpr:
		...
		// degeneracy has to be flagged explicitly instead: tag the result
		// with degenerateRootMarker so isOpaqueRoot can route it to
		// unanchored rather than classify reading its accidental leading
		// slash as the same positive evidence a genuine literal like
		// "/etc/cogos/config.yaml" carries.
		if left == "" && strings.HasPrefix(right, "/") {
			combined = degenerateRootMarker + combined
		}
		return combined, leftOK && rightOK
...
func scanParsedFile(pf parsedFile, idx *globalIndex) []Site {
	...
	ast.Inspect(pf.file, func(n ast.Node) bool {
		...
		sites = append(sites, Site{
			...
			Primitive: primitive,
			Func:      sc.label,
			Pattern:   stripDegenerateRootMarker(pattern),
			Category:  category,
			Resolved:  resolvedOK,
...
// stripDegenerateRootMarker removes a leading degenerateRootMarker from
// pattern, if present, before the pattern is stored on a Site and
// surfaced to a human in the rendered inventory. The marker's only job is
// steering classify away from a false "elsewhere" (see its own doc
// comment and isOpaqueRoot below); once classification has happened it
// must not leak into the committed golden or rendered markdown as a
// literal NUL-delimited string.
func stripDegenerateRootMarker(pattern string) string {
	return strings.TrimPrefix(pattern, degenerateRootMarker)
}

func isOpaqueRoot(pattern string) bool {
	if strings.HasPrefix(pattern, degenerateRootMarker) {
		// A concatenation whose left operand resolved to "" and whose
		// right-hand literal happened to begin with "/" — see
		// degenerateRootMarker's doc comment. The leading slash this
		// pattern appears to have is exactly the same artifact
		// filepath.Join(emptyBase, name) used to leave behind before the
		// Join handler learned to skip empty parts; it is not a
		// positively-established absolute-path root.
		return true
	}
	if pattern == "" {
