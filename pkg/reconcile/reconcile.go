// Package reconcile is the legacy import path for what is now the canonical
// package at github.com/myrgic/cogos/pkg/substrate/reconcile.
//
...
// implementation lives at pkg/substrate/reconcile/. This package retains the
// legacy import path as a thin re-export shim so external consumers
// (federation peers, language bindings, operator scripts) that still import
// the legacy path continue to compile without source changes.
//
// All symbols here are Go type aliases or variable aliases of their
// counterparts in github.com/myrgic/cogos/pkg/substrate/reconcile. Consumers
// of either import path get identical types at the language level. No
// conversion is needed at the boundary.
//
// This shim adds no logic. New code should import the substrate path directly.
package reconcile

import reconcile "github.com/myrgic/cogos/pkg/substrate/reconcile"
