// Package identity is the substrate-canonical home for the Identity CRD
// schema — the OIDC-compatible Kubernetes-style identity definition, its
// supporting types (KeyRef, AuthFactor, VoiceProfile, Expression), the
// YAML loader, and the validation logic.
//
// Per ADR-100 Step 3, the schema types extracted from the root
// identity_crd.go file live here. The reconciler (IdentityProvider in
// identity_provider.go) remains kernel-resident because it depends on
// internal/engine runtime types.
//
// Per RFC-034 §3.3 (diagnostic rule): "if the behavior can be tested
// without a running process, it belongs in the substrate library." Every
// function in this package is a pure CPU-bound operation on bytes or
// structs; no daemon, no goroutine, no I/O beyond reading YAML files.
//
// Shape is OIDC-compatible: each identity has a stable (iss, sub) anchor
// and a list of expressions keyed by audience (workspace, channel,
// resource). The reconciler's "collapse operation"
// (CRDSpec.ExpressionFor) picks the expression matching the current
// audience when projecting — that's how global identity maps down to its
// contextual local expression.
//
// Identity is a CRD managed by the CogOS reconciliation loop
// (pkg/reconcile). The spec on disk is one leg of the
// (Spec, Projections, Reconciler) triple that defines an identity at
// runtime.
package identity
...
// CRD is the top-level Kubernetes-style identity definition.
type CRD struct {
...
// CRDMeta matches the standard CRD metadata shape.
type CRDMeta struct {
...
// CRDSpec holds the OIDC-shaped identity body.
//
...
// Expression is one projection of an identity into a specific audience.
...
// APIVersion and Kind — validated on load to catch typos early.
const (
...
// crdDir returns the path to the identity definitions directory.
// Matches the `.cog/config/<provider>/` convention used by services.
func crdDir(root string) string {
	return filepath.Join(root, ".cog", "config", "identities")
}

// CRDDir returns the conventional path to the identity definitions directory
// for the given workspace root. Exposed so kernel-side code can reference
// the same path without duplicating the convention.
func CRDDir(root string) string {
	return crdDir(root)
}
...
// LoadCRDs loads every identity CRD under
...
// diffs stay stable.
func LoadCRDs(root string) ([]*CRD, error) {
	dir := crdDir(root)
	...
	for _, e := range entries {
		...
		crd, err := loadCRDFile(filepath.Join(dir, name))
		if err != nil {
...
// loadCRDFile reads + parses + validates one CRD file.
func loadCRDFile(path string) (*CRD, error) {
...
// ValidateCRD enforces the invariants the reconciler depends on.
// Fail fast at load time so the plan/apply loop never sees malformed specs.
// Exposed so kernel-side validation paths (test helpers, idempotent re-checks)
...
func ValidateCRD(crd *CRD) error {
	...
	if crd.APIVersion != APIVersion {
		return fmt.Errorf("apiVersion = %q, want %q", crd.APIVersion, APIVersion)
	}
	if crd.Kind != Kind {
		return fmt.Errorf("kind = %q, want %q", crd.Kind, Kind)
	}
	...
	if _, ok := validTypes[crd.Spec.Type]; !ok {
...
//
// Exposed so kernel-side resolvers can use the same parsing logic.
...
// parseKeyRefScheme is the unexported alias kept for internal package
// callers (validators). Exported variant is ParseKeyRefScheme.
func parseKeyRefScheme(ref string) (string, bool) {
	return ParseKeyRefScheme(ref)
}
...
// AllowedKeyRefSchemes lists the URI schemes recognized for KeyRef.Ref.
// Exposed so kernel-side resolver registrations stay in lockstep with the
// schema's allow-list. New schemes are added here as resolvers are
// implemented in identity_provider.go.
...
// allowedKeyRefSchemes is the unexported alias retained for internal use.
var allowedKeyRefSchemes = AllowedKeyRefSchemes
...
// sortStrings is a tiny stable-sort helper so this package does not
...
func (s *CRDSpec) ExpressionFor(aud string) *Expression {
	if s == nil {
		return nil
	}
