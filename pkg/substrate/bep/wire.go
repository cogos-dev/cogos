...
//
// Extracted from apps/cogos/bep_wire.go.
...
const (
	...
	MaxHelloSize   = 32768    // 32 KiB
)

// Wire handles reading and writing BEP-framed messages on a connection.
type Wire struct {
...
// NewWire wraps a connection with BEP framing.
...
func (w *Wire) WriteHello(h *Hello) error {
	w.writeMu.Lock()
	defer w.writeMu.Unlock()

	payload := h.Marshal()
	if len(payload) > MaxHelloSize {
		return fmt.Errorf("hello too large: %d > %d", len(payload), MaxHelloSize)
	}
...
func (w *Wire) ReadHello() (*Hello, error) {
	w.readMu.Lock()
...
func (w *Wire) WriteMessage(msgType MessageType, payload []byte) error {
	w.writeMu.Lock()
	defer w.writeMu.Unlock()

	// Encode header.
	header := &Header{Type: msgType, Compression: CompressionNone}
...
func (w *Wire) ReadMessage() (MessageType, []byte, error) {
	w.readMu.Lock()
	defer w.readMu.Unlock()
