// index.go — Version vectors, local index scanning, diff computation,
// and index persistence for BEP agent CRD sync.
//
// Extracted from apps/cogos/bep_index.go.
...
func (v *VersionVector) ToBEP() *Vector {
	bv := &Vector{}
	for id, val := range v.Counters {
		bv.Counters = append(bv.Counters, &Counter{ID: id, Value: val})
	}
...
func VersionVectorFromBEP(bv *Vector) *VersionVector {
	v := NewVersionVector()
	if bv != nil {
...
// ToBEPFileInfo converts an IndexEntry to a FileInfo for wire transmission.
func (e *IndexEntry) ToBEPFileInfo(shortID uint64) *FileInfo {
	fi := &FileInfo{
		Name:       e.Name,
		Size:       e.Size,
...
// IndexEntryFromBEP creates an IndexEntry from a received FileInfo.
func IndexEntryFromBEP(fi *FileInfo) *IndexEntry {
	return &IndexEntry{
		Name:       fi.Name,
		Size:       fi.Size,
